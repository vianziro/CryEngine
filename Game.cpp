...
//Serkan
RTSCamera RTSCamera_01;
SQLFederate *sqlfed=new SQLFederate();


//SpeechRecognition SpeechRecognition_01;
...
int CGame::Update(bool haveFocus, unsigned int updateFlags)
{
  ...
  Stereo3D::Update(frameTime);
	...
	CheckReloadLevel();
	//Added by sekogs
	std::string LevelName=RTSCamera_01.GetLevelName();

	if(LevelName != EmptyString)
	{
		sqlfed->startConnection();
		sqlfed->setSimulationName("asdasd");
		sqlfed->setStartingTime("12.00");
		sqlfed->setWeather("parcali");
		//sqlfed->startHLA();
