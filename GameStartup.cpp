...
LRESULT CALLBACK CGameStartup::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		if(gEnv && gEnv->pHardwareMouse)
		{
			...
			MouseTry *MouseTry_1=0;
			MouseTry_1->MouseEvent(LOWORD(lParam),HIWORD(lParam),HARDWAREMOUSEEVENT_LBUTTONDOWN);
		}
		return 0;
	case WM_LBUTTONUP:
		if(gEnv && gEnv->pHardwareMouse)
		{
			...
			MouseTry *MouseTry_1=0;
			MouseTry_1->MouseEvent(LOWORD(lParam),HIWORD(lParam),HARDWAREMOUSEEVENT_LBUTTONUP);
		}
		return 0;
	...
	case WM_RBUTTONDOWN:
		if(gEnv && gEnv->pHardwareMouse)
		{
			gEnv->pHardwareMouse->Event(LOWORD(lParam),HIWORD(lParam),HARDWAREMOUSEEVENT_RBUTTONDOWN);
		}
		return 0;
	...
	case WM_RBUTTONUP:
		if(gEnv && gEnv->pHardwareMouse)
		{
			gEnv->pHardwareMouse->Event(LOWORD(lParam),HIWORD(lParam),HARDWAREMOUSEEVENT_RBUTTONUP);
			MouseTry *MouseTry_1=0;
			MouseTry_1->MouseEvent(LOWORD(lParam),HIWORD(lParam),HARDWAREMOUSEEVENT_RBUTTONUP);
	...
	case WM_RBUTTONDBLCLK:
		if(gEnv && gEnv->pHardwareMouse)
		{
			gEnv->pHardwareMouse->Event(LOWORD(lParam),HIWORD(lParam),HARDWAREMOUSEEVENT_RBUTTONDOUBLECLICK);
		}
		return 0;
  ...
  case WM_STYLECHANGED:
	...
	case WM_INPUTLANGCHANGE:
		...
		switch (wParam)
		{
			case VK_LEFTARROW:
				{
					g_RTSKeyboard.KeyboardEvent(EHARDWAREKEYBOARDEVENT_LEFT_KEYPRESSED);
				}
			break;

			case VK_RIGHTARROW:
				{
					g_RTSKeyboard.KeyboardEvent(EHARDWAREKEYBOARDEVENT_RIGHT_KEYPRESSED);
				}
			break;

			case VK_UPARROW:
				{
					g_RTSKeyboard.KeyboardEvent(EHARDWAREKEYBOARDEVENT_UP_KEYPRESSED);
				}
			break;

			case VK_DOWNARROW:
				{
					g_RTSKeyboard.KeyboardEvent(EHARDWAREKEYBOARDEVENT_DOWN_KEYPRESSED);
				}
			break;
		}
